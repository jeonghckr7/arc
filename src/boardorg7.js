        ...
        if (fileNameMatch) {
          ...
          function onReject(error) {
              ...
              .then(snap => {
                ...
                //return streffile.getDownloadURL()
...
async function NoIdBoard(id, no, docid) {
  ...
  //alert("idboard: " + id+ "noidboard: " + no + "newPostKey" + newPostKey);
    ...
    await axios.get(cors_api_url + 'https://gall.dcinside.com/board/view/?id=' + id + '&no=' + no +'&exception_mode=recommend')
    .then(function(iresult) {
      const html = iresult.data;
      
      var patternorg = /loading_btntype[\s\S]*?([^<]+?)<\/div>/i;
      ...
      while((imgsrcs = reg_imgsrc.exec(wr_content)) !== null) {
        console.log("imgsrcs" + imgsrcs[1])
        const imgsrcs_array = imgsrcs[1].split('&');
        var imgsrcsr = opimage_array[0] + "&" + imgsrcs_array[1]
        //console.log({imgsrc: imgsrcsr})
        //igs.push(imgsrcsr);
        igs.push({imgs:imgsrcs[1] ,imgscr: imgsrcsr, ogsname: ogs[wcount]});
        wcount++;
      }
